...
$image = $avatar->name($input->name)
    ...
    ->size($input->size)
    ->background(empty($_GET['input->background'] == false) ? $input->background : colorByName($input->name))
    ->color($input->color)
    ->smooth()
