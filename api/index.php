...
$image = $avatar->name($input->name)
    ...
    ->size($input->size)
    ->background(array_key_exists("background", $_GET) ? $input->background : colorByName($input->name))
    ->color($input->color)
    ->smooth()
